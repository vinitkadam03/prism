...
class OtlpDriver implements TelemetryDriver
{
    ...
    protected function provider(): TracerProvider
    {
        ...
        return $this->provider = new TracerProvider(
            ...
            spanLimits: (new SpanLimitsBuilder)
                ->setAttributeCountLimit((int) ($this->config['attribute_count_limit'] ?? 10_000))
                ->setAttributePerEventCountLimit((int) ($this->config['attribute_per_event_count_limit'] ?? 10_000))
                ->build(),
    ...
    protected function createSpanProcessor(SpanExporterInterface $exporter): SpanProcessorInterface
    {
        ...
        return $container->make($processorClass, [
            'exporter' => $exporter,
